...
Server-server streaming may be required for inter-process communication or for when a device is attached to the computer, say via a USB port, and the Meteor App is required to access it via a local server running as a background process.
...
Now you can open 2 browser tabs/windows and chat using sendMessage("text") at your browser's console Every message will travel from your client to server and retransmitted to all other clients.
